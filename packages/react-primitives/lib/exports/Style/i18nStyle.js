...
const additiveInverse = (value) => multiplyStyleLengthValue(value, -1);
  ...
  for (const originalProp in style) {
    ...
    if (prop === "transitionProperty") {
      ...
      if (PROPERTIES_I18N.hasOwnProperty(value)) {
        ...
        const flippedValue = PROPERTIES_FLIP[originalValue];
    ...
    if (isRTL && prop === "textShadowOffset") {
      const invertedValue = additiveInverse(value.width);
